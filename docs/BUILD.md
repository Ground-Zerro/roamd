...
|---|---|---|
| `roamd` | демон, init-скрипт, конфигурация, ACL канала управления, скрипты mesh | `libubox`, `libubus`, `libuci`, `libblobmsg-json` |
...
(полный `wpad` ставится автоматически), `dropbear` (SSH-клиент и `dropbearkey`), `curl` на
контроллере с узлами, `openssl-util` на узле (ставится при захвате).
...
| Файл | Строка |
|---|---|
| `package/roamd/Makefile` | `PKG_VERSION:=1.0.1-r14` |
| `package/luci-app-roamd/Makefile` | `PKG_VERSION:=1.0.1-r6`, `PKG_PO_VERSION:=$(PKG_VERSION)` |

* Пакеты нумеруются независимо: изменился демон или скрипты — поднимается версия `roamd`,
...
начинаются с ветки и архитектуры (`25.12-x86_64-roamd-1.0.1-r14.apk`).
