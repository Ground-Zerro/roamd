...
static void sha256_update(struct sha256 *s, const uint8_t *p, size_t len)
{
	s->count += len;

	while (len) {
		size_t take = sizeof(s->buf) - s->used;
...
static void sha256_final(struct sha256 *s, char *hex)
{
	...
	for (i = 0; i < 8; i++)
		snprintf(hex + i * 8, 9, "%08x", s->state[i]);
}
